...
#include "stdafx.h"
#include "FV1.h"	 //View public the DisplayView
...
void CSplitterControl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_bIsPressed)
	{
		...
		CPoint pt(m_nX, m_nY);
		m_bIsPressed = FALSE;
		//Get the parent window ,and set the split new position
		...
		if (pOwner && IsWindow(pOwner->m_hWnd))
		{
			...
			//notice partent dlg ,modify the size of view
			if (m_nType == SPS_VERTICAL)
			{
				m_rect0.right=pt.x;
				m_rect1.left=pt.x+3;
			...
			FV1 *dlg=static_cast<FV1 *>(pOwner);
